...
|---|---|---|---|---|---|---|---|---|
| P0-1 | Sécurité | `staff_profiles_public_select` RLS policy exposes `invitation_token`+`phone` to any unauthenticated request — sole credential `accept-invitation` uses to bind an account to a staff role/salon; account-takeover vector. Confirmed live in production via read-only `pg_policy` check. Fix drafted (`20260704190000...sql`), live-tested before/after on dr-scratch; **a real bug was found in the first draft itself** (`security_invoker=true` silently emptied the practitioner picker for every client) and corrected. | P0 | Critical (CVSS not computed by original finder; account-takeover class) | None — deploy-ready | 15 min apply + smoke test | Live before/after exploit test, `PHASE_2_SECURITY_FIXES.md` | Corrigé-non-déployé |
...
| P1-2 | Backend / Infrastructure | **21 migrations never deployed to production** (confirmed via `supabase migration list --linked`, 2026-07-05: 81 local, 59 applied, 21 unapplied — grew from 20 to 21 this pass with the addition of the P2-24 realtime-publication fix, Master Plan Execution CP2). 0 classified BLOCKER across all 21. Full classification and apply order in §7/`CP2_DEPLOYMENT_READY.md`. | P1 | High — root cause of every low Monitoring/Observability/Production-Readiness score in this program | See §7 dependency chain | ~1 day (apply+verify all 21) | `MIGRATION_APPLICATION_PLAN.md`, `MIGRATION_REVIEW.md`, `docs/master-plan-execution/CP2_DEPLOYMENT_READY.md`, direct CLI re-check 2026-07-05 | Corrigé-non-déployé |
| P1-3 | Infrastructure / DR | Recurring automated backup mechanism **built and live-tested this pass** (Master Plan Execution CP3): draft migration `20260705130000_cp3_platform_backup_automation.sql` + new Edge Function `create-platform-backup`, run twice on `kynza-dr-scratch` (78 tables, 57,278 rows each run), `pg_cron` job `kynza-platform-backup` genuinely registered (`0 */6 * * *`), a 6-table/5,087-row restore rehearsal proved exact-match restorability. **On dr-scratch, RPO is now bounded at ≤6h, not rising.** In production, RPO remains ≈18.5h+ and rising until this migration/function is deployed with Mylord's approval (same gate as every other item in this table) — see `CP3_ENGINEERING_CLOSURE.md` for full evidence including a real bug found and fixed live (a 400,001-row stale scale-test table on dr-scratch initially exhausted the function's compute budget; fixed with an honest per-table truncation cap). | P1 | High until deployed | None | 2-4h (cron job + service-role backup variant) — **done, this pass** | `PHASE_0_BACKUP_CONFIRMED.md`, `DISASTER_RECOVERY_REPORT.md` (CP4), `docs/master-plan-execution/CP3_ENGINEERING_CLOSURE.md`, 2 real live runs + restore rehearsal 2026-07-05 | Corrigé-non-déployé |
| P1-4 | Android / Release | No real Android upload keystore exists (`android/key.properties` and `android/app/upload-keystore.jks` both confirmed absent, 2026-07-05). The **conditional signing wiring is real and correct** (`build.gradle.kts` falls back to debug signing only when the real file is absent, verified end-to-end with a disposable test keystore) — this is why the Hardening pass correctly said "Release signing ✅ resolved" while Certification v2 correctly said "no real keystore provisioned": both are true, they describe different halves of the same fact. See §6 for the full resolution. One-way secret; Mylord-only action. | P1 | High — hard Play Store blocker | None (deliberately deferred, needs custody plan first) | 30 min (keytool) + custody planning | `docs/android/RELEASE_SIGNING_PROCEDURE.md`, `PHASE_4_READINESS_CLOSURES.md §1`, direct re-check 2026-07-05 | Ouvert |
...
| P1-8 | Play Store | Data Safety Form never started in Play Console. | P1 | High (Play blocker) | None | 1-2h, business decision | `CP9_STORE_GO_NO_GO.md` | Ouvert |
| P1-9 | Fiabilité / Concurrency | Two real concurrency bugs: `OfflineSyncCoordinator.flush()` double-applies every queued mutation on concurrent flush; `run-scheduled-actions` double-processes a pending automation action (would double-send a real WhatsApp/push) on concurrent cron invocation. Found in Final Enterprise Validation (explicitly **not fixed there** — committed as a skipped regression test, deferred as "P1.5"), then **fixed** in Enterprise Resilience CP0: client fix (`AtomicClaimService`) ships with the next app release; server fix is draft migration `20260705100000_cp0_concurrency_atomic_claims.sql`, live-tested on dr-scratch, not applied to production. 2 more of the same bug shape found and fixed in the same pass (`LegalAcceptanceService` double-flush; `schedule-reminders` TOCTOU). | P1 | High once real traffic exists ("low-risk only because production traffic is near zero" — `PRODUCTION_READINESS_FINAL.md §3`) | Client fix: none. Server fix: part of P1-2's 20-migration batch | Client: ships automatically next release. Server: 15 min apply | `OFFLINE_REPORT.md`, `BACKGROUND_JOBS_REPORT.md`, `CONCURRENCY_REPORT.md` (CP0), live RPC race test on dr-scratch | Corrigé-non-déployé (server); Corrigé, awaiting next release (client) |
...
| P1-12 | Observabilité | Payment-failure dashboard + full alerting mechanism (3 thresholds: Edge Function error rate, sync queue staleness, payment failure rate) built and proven live on `kynza-dr-scratch` — all 3 simulated concurrent incidents caught on the first check, dedup confirmed. Draft migration `20260705110000_cp6_observability_alerting.sql`, not applied to production. WhatsApp-dispatch half not live-tested end-to-end (needs live credentials/recipient). No `pg_cron` schedule created yet — needs a production target. | P0 (per Resilience pass's own recommendation, newly elevated) | High | Depends on `20260704120000` (Health Center base) already being applied — part of P1-2 batch | 30 min apply + smoke test; cron schedule TBD | `OBSERVABILITY_ADVANCED_REPORT.md` | Corrigé-non-déployé |
...
| P2-3 | Sécurité | `run-scheduled-actions`/`schedule-reminders` rely on `verify_jwt:true` alone — satisfied by the public anon key shipped in the app bundle, not a real "cron-only" guarantee. Fix drafted (`X-Cron-Secret` header check + `20260704220000...sql`), live-tested before/after — **a real bug found**: drafted migration assumed wrong `pg_cron` job names (`schedule-reminders-hourly`/`run-scheduled-actions-5min`); real names (`kynza-booking-reminders`/`kynza-run-scheduled-actions`) confirmed and corrected. **Hard precondition before applying**: `CRON_SECRET` must be set as both an Edge Function secret and a Vault entry in production *first*, or reminders/automation silently stop firing with no visible error. | P2 | Medium (idempotency guards already limit blast radius) | Must set `CRON_SECRET` in prod before applying migration — see §7 | 30 min, in exact order | `CP4_EDGE_FUNCTION_REVERIFY.md`, `PHASE_2_SECURITY_FIXES.md` | Corrigé-non-déployé |
| P2-4 | Sécurité / Database | 2 `SECURITY DEFINER` views bypass caller permissions (`v_popular_searches`, `v_mv_daily_revenue`). | P2 | Medium | None | Needs design decision, not drafted | `PHASE_2_DATABASE_OPTIMIZATION.md` (CP2 advisor) | Ouvert |
| P2-5 | Sécurité | Oversized payload (2MB JSON) hangs an Edge Function 45+ seconds; no function has a body-size limit. | P2 | Medium (mild DoS) | None | Small, per-function | `PHASE_6_SECURITY_OFFENSIVE.md` | Ouvert |
...
| P2-7 | Observabilité | `activity_logs.ip_address`/`device_info` not populated by several Edge Functions (`accept-invitation`, `calculate-commission`, `claim-referral` spot-checked). Systemic, not fully audited. | P2 | Medium (audit-quality gap) | None | Medium (per-function) | `CP1_ARCHITECTURE_REVERIFY.md`, `CP5_OBSERVABILITY_MONITORING_GAP.md` | Ouvert |
| P2-8 | Sécurité | `is_system_admin` has no documented grant/revoke/audit mechanism — anyone with direct DB access can flip it with no trail. | P2 | Medium | Should land with/after P1-2 | Small | `MASTER_ISSUES_MATRIX.md` P2-8 | Ouvert |
...
| P2-10 | Tests | 23.29% line coverage overall; **zero repository-layer test files exist at all** — no DI/mocking seam for any repository wrapping `SupabaseService` directly. | P2 | Medium-high (structural, not a specific bug) | New mocking infrastructure needed | Large | `PHASE_9_ENTERPRISE_TESTING_COVERAGE.md`, `CERTIFICATES/QA.md` | Ouvert |
| P2-11 | Backend | `proxipay-create-session` has no unique constraint on `booking_id` — **the single most-repeated never-fixed finding in this entire program (flagged independently by 5 separate passes)**. | P2 | Medium | None | Small (add constraint) | `MASTER_ISSUES_MATRIX.md` P2-11 (5× corroboration) | Ouvert |
...
| P2-13 | Sécurité | `PermissionGuard` built, wired into zero screens. | P2 | Low-medium | None | Medium | `MASTER_ISSUES_MATRIX.md` P2-13 | Ouvert |
...
| P2-15 | Database / Performance | 32 unindexed foreign keys across 24 tables (27 new + 5 pre-existing). Fix drafted, bundled in P1-2 batch (`20260703120000` + `20260704180000`). | P2 | Medium | Part of P1-2 | Included in P1-2's 15 min | `PHASE_2_DATABASE_OPTIMIZATION.md`, `MIGRATION_APPLICATION_PLAN.md` | Corrigé-non-déployé |
| P2-16 | Database / Performance | 83 `auth_rls_initplan` advisor warnings across 49 tables (RLS calling `auth.uid()` directly instead of `(select auth.uid())`, causing per-row re-evaluation). | P2 | Medium at scale | Needs per-policy review, not a blind rewrite | Large | `PHASE_2_DATABASE_OPTIMIZATION.md`, `SQL_PERFORMANCE_REPORT.md` (re-confirmed open) | Ouvert |
| P2-17 | Database / Performance | 205 `multiple_permissive_policies` advisor warnings across 23 tables. | P2 | Medium at scale | None | Large | `PHASE_2_DATABASE_OPTIMIZATION.md`, `SQL_PERFORMANCE_REPORT.md` (re-confirmed open) | Ouvert |
| P2-18 | Database | Missing `updated_at` trigger despite the column existing: `salon_settings`, `permission_groups`, `automation_workflows` — "a real correctness bug, not a design choice," never fixed across 5 passes. | P2 | Low-medium | None | Trivial (3 triggers) | `DATABASE_ARCHITECTURE.md`, `MASTER_ISSUES_MATRIX.md` P2-18 | Ouvert |
| P2-19 | Business | Bank transfer details still literal `[À CONFIGURER]` placeholder in `KynzaConstants` and `create-manual-invoice`. | P2 | Medium (blocks real invoicing) | None | Business-owned | `PRODUCTION_CHECKLIST.md`, `MASTER_ISSUES_MATRIX.md` P2-19 | Ouvert |
...
| P2-21 | Sécurité | Certificate pinning scaffolded but permanently inert (`featureFlagEnabled=false`, empty cert bytes); root/jailbreak detection not implemented. | P2 | Medium | Needs a verified capture of Supabase's real cert from a trusted environment first | Medium | `SECURITY_AUDIT_V2.md`, `MASTER_ISSUES_MATRIX.md` P2-21 | Ouvert |
| P2-22 | Scalabilité | Real bulk-write ceiling found: a per-row `UPDATE` trigger (`trg_increment_monthly_bookings`) caps any single bulk-insert statement around 150,000-300,000 rows before hitting a ~2-minute statement timeout. Confirmed via real 400,001-row test (bisected in batches to avoid the ceiling). 100k-client/20k-staff/1M-booking tiers never reached (would need ~11 more minutes of batching + a materially different `auth.users` seeding approach). Not re-tested in the Resilience pass (carried forward unchanged). | P2 | Medium — real, concrete, but currently far above any plausible near-term traffic | None | Medium (batch or async the trigger) | `SCALABILITY_REPORT.md` (CP6), `FINAL_CERTIFICATION.md` (carried forward) | Ouvert |
| P2-23 | Performance / Realtime | 3 unbounded Realtime `.stream()` call sites (booking calendar salon/practitioner variants, notifications list) fetch a practitioner's/user's entire history client-side with no server-side bound. Measured 46× slower (1.4ms→64ms) at 400k rows vs. the properly-bounded equivalent, which only slowed 3× for the same 40× data growth. | P2 | Medium, will compound with real growth | None | Medium (add bound + pagination) | `SQL_PERFORMANCE_REPORT.md`, `SCALABILITY_REPORT.md` | Ouvert |
| P2-24 | Realtime | `notification_logs` is not in the `supabase_realtime` publication — new notifications don't appear live on an open screen (initial snapshot loads correctly). **Drafted and applied to `kynza-dr-scratch` this pass** (Master Plan Execution CP2): `20260705120000_cp2_realtime_notification_logs.sql`. DDL succeeded without a duplicate-member error, confirming the table is now genuinely a publication member — a full Realtime-client round-trip (subscribe/mutate/observe) was **not** performed (no `@supabase/supabase-js` tooling set up in this Flutter repo), flagged honestly as a partial proof, not silently upgraded. | P2 | Medium (UX gap) | None | Trivial (add to publication) — **drafted, this pass** | `REALTIME_REPORT.md`, `docs/master-plan-execution/CP2_DEPLOYMENT_READY.md` | Corrigé-non-déployé |
...
| P2-26 | Sécurité | `check_rate_limit` RPC fails open (`if (error) return true`) — an outage/bug in the limiter itself lets requests through rather than blocking them. | P2 | Medium | None | Small | `SECURITY_REPORT.md` (CP7) | Ouvert |
| P3-1 | Architecture | 3 real `core`↔`feature` circular provider dependencies (`auth_providers.dart`↔`auth_notifier.dart`; `offline_sync_providers.dart`↔3 feature providers). Compiles/works, maintainability debt only. | P3 | Low | None | Medium (refactor) | `CP1_ARCHITECTURE_REVERIFY.md` | Ouvert |
...
| P3-5 | Edge Functions | Systemic hygiene gap: 0/20 functions have an explicit timeout, ~1/20 have metrics, 0/20 have tracing/correlation-IDs, ~0/20 have structured logging. | P3 | Low-medium | None | Large (per-function) | `PHASE_3_EDGE_FUNCTION_CERTIFICATION.md`, `CP5_OBSERVABILITY_MONITORING_GAP.md` | Ouvert |
| P3-6 | Backend | 8 pre-existing unbounded repository stream/fetch methods (distinct from P2-23's Realtime-specific 3). | P3 | Low | None | Medium | `MASTER_ISSUES_MATRIX.md` P3-6 | Ouvert |
...
| P3-8 | Database | `salon_settings`/`owner_journey_progress`/`referrals` missing `deleted_at`. | P3 | Low | None | Trivial | `DATABASE_ARCHITECTURE.md` | Ouvert |
| P3-9 | Database | `salons.owner_id` not a declared FK, no index, despite being used throughout RLS. | P3 | Low | None | Small | `DATABASE_ARCHITECTURE.md` | Ouvert |
...
| P3-11 | Ops | No in-app/admin UI to create a maintenance window — SQL-only today. | P3 | Low | None | Small | `PRODUCTION_CHECKLIST.md` | Ouvert |
| P3-12 | Observabilité | Crash Dashboard / Performance Dashboard have no queryable data source (Firebase Performance has no read API) — platform limitation, not a code bug. | P3 | Low | None | N/A (platform limit) | `PHASE_2_OBSERVABILITY.md`, `PHASE_4_PERFORMANCE_OBSERVABILITY.md` | Non validé |
| P3-13 | Auth | Facebook and Apple sign-in are both stubs (`throw UnimplementedError`). | P3 | Low | None | Medium | `PRODUCTION_CHECKLIST.md` | Ouvert |
| P3-14 | Product | No Google Maps/Places/Geolocation, Firebase Analytics, or local-notifications package integrated. | P3 | Low | None | Large | `PRODUCTION_CHECKLIST.md` | Ouvert |
...
| P3-16 | Database | `rls_enabled_no_policy` advisor warning on `rate_limit_buckets` (RLS enabled, zero policies — correctly default-deny, but never given an explicit written verdict). | P3 | Low | None | Trivial (write the verdict) | `PHASE_2_DATABASE_OPTIMIZATION.md` | Non validé |
| P3-17 | Storage | `public_bucket_allows_listing` advisor warning on `kynza-media` — likely intentional (public menu images), never given an explicit written verdict. | P3 | Low | None | Trivial | `PHASE_2_DATABASE_OPTIMIZATION.md` | Non validé |
...
| P3-19 | Database | 50 `unused_index` advisor warnings — explicitly not actionable pre-launch (false signal at near-zero traffic). | P3 | Low | None | N/A pre-launch | `PHASE_2_DATABASE_OPTIMIZATION.md` | Non validé |
...
**11 are closed with proof** (`Fermé (preuve)`: R-1 through R-7, P1-5/CI-CD, P1-11/CMS-cache-invalidation, and 2 newly closed this pass — **P1-13** (cold-start-offline read cache, built and live-tested) and **P2-6/P2-27** (MANAGER/SYSTEM_ADMIN RLS isolation, tested for the first time with real QA fixtures)). **15 have a fix that exists and is live-tested on staging but not in production** (`Corrigé-non-déployé`) — the original 12 plus 3 newly closed to this state this pass: **P2-9** (Remote Config admin gate, drafted+tested) and **P2-24** (notification_logs Realtime publication, drafted+tested), plus **P1-3** upgraded from "one-time backup only" to a genuinely built-and-tested recurring mechanism (still `Corrigé-non-déployé` pending production deploy approval, but no longer just "the residual is unaddressed"). The remaining **42 are genuinely open or never independently tested** (`Ouvert`/`Non validé`) — down from 47, a net reduction of 5 rows moved to a stronger state, zero rows moved backward.
