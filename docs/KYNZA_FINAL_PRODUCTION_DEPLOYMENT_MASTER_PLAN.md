...
| P1-2 | Backend / Infrastructure | **27 migrations never deployed to production** (re-confirmed via `supabase migration list --linked`, 2026-07-06: 86 local, 59 applied, 27 unapplied — grew from 21 across Enterprise Final 100's own CP2/CP3/CP4/CP7/CP8, each adding one real migration: `20260706100000`, `20260706110000`, `20260706120000`, `20260706130000`, `20260706140000`. **The "21" figure this row previously stated was stale as of the Final Engineering Certification reconciliation pass — corrected here, a genuine inconsistency found while verifying, not a new finding.**). 0 classified BLOCKER across all 27. Full classification and apply order in §7/`CP2_DEPLOYMENT_READY.md`/`CP7_PRODUCTION_READINESS.md`. | P1 | High — root cause of every low Monitoring/Observability/Production-Readiness score in this program | See §7 dependency chain | ~1 day (apply+verify all 27) | `MIGRATION_APPLICATION_PLAN.md`, `MIGRATION_REVIEW.md`, `docs/master-plan-execution/CP2_DEPLOYMENT_READY.md`, `docs/enterprise-final-100/CP7_PRODUCTION_READINESS.md`, direct CLI re-check 2026-07-06 | Corrigé-non-déployé |
...
| P2-8 | Sécurité | `is_system_admin` has no documented grant/revoke/audit mechanism — anyone with direct DB access can flip it with no trail. **Fixed (Enterprise Final 100 CP2)**: new `grant_system_admin()`/`revoke_system_admin()` RPCs + `system_admin_audit` table. Live-tested end-to-end on dr-scratch (forbidden for non-admins, success + audit row for admins, both grant and revoke) — caught and fixed a real bug along the way (`protect_user_columns` would have rejected the RPC's own UPDATE without a transaction-local flag). | P2 | Medium | Should land with/after P1-2 | Small | `MASTER_ISSUES_MATRIX.md` P2-8, `CP2_SECURITY.md` | Corrigé-non-déployé |
...
| P2-11 | Backend | `proxipay-create-session` has no unique constraint on `booking_id` — **the single most-repeated never-fixed finding in this entire program (flagged independently by 5 separate passes)**. **Fixed (Enterprise Final 100 CP7)**: partial unique index (`booking_id WHERE status='pending'`). Live-tested on dr-scratch, both directions: a second concurrent pending session for the same booking is rejected (23505); a legitimate retry after cancellation still succeeds. | P2 | Medium | None | Small (add constraint) | `MASTER_ISSUES_MATRIX.md` P2-11 (5× corroboration), `CP7_PRODUCTION_READINESS.md` | Corrigé-non-déployé |
