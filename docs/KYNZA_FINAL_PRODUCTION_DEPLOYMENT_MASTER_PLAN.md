...
> ## ⚠️ CURRENT STATE AS OF 2026-07-07 (Backend Governance Phase 1) — READ THIS BEFORE §1/§19/§20
> **§1 (Executive Summary), §19, and §20 below describe the state *before* the 2026-07-06 go-live
> deployment and were never revised after §2's rows were updated in place.** This created a real
> internal inconsistency (found by `docs/final-doc-verification/FINAL_DECISION.md`, Question 3):
> §2 correctly shows P0-1/P1-1/P1-2/P1-3/P1-5/P1-12 as `Fermé (preuve)`, while §1/§19/§20 still
> narrate P0-1 as "unpatched... today" and list closing P0-1/P1-2/P1-3/P1-4 as a still-pending
> trigger condition. **The correct, current state, per §2 and live re-verification
> (`docs/final-doc-verification/P0_VERIFICATION.md`, `P1_VERIFICATION.md`, both 2026-07-07):**
> - **P0-1: closed**, live in production, re-verified today by direct `pg_policies` query and a
>   real unauthenticated exploit attempt returning `[]`.
> - **P1-1, P1-2, P1-3, P1-5, P1-9 (server), P1-11, P1-12, P1-13: closed**, live in production.
> - **P2-2, P2-5, P2-9 and every other row §2 marks `Fermé (preuve)`: closed**, per §2 itself.
> - **The only P0/P1-severity items still open are P1-4, P1-6, P1-7, P1-8 — all four are External
>   Go-Live Dependencies** (real Android keystore, real legal content, Apple Developer enrollment,
>   Play Store Data Safety form), **not open engineering or operations work.**
> - **P2-5's own row is now further updated** (Backend Governance Phase 1, citing the P2-5 RCA and
>   ECR, both 2026-07-07): closed with engineering evidence, precisely scoped. A new, separate,
>   still-open item (**P2-28**, formerly misfiled as `P2-22` in
>   `docs/remediation/MASTER_ISSUES_MATRIX.md` before this session's renumbering) was discovered
>   while validating P2-5's fix and is tracked in §2 below.
> - **§1's "Go/No-Go: No-Go" and §20's trigger conditions are stale** — 3 of the 4 conditions §20
>   lists (P0-1, the migration batch, the recurring backup) are already met per §2. Only P1-4 (the
>   Android keystore, a pure Mylord action with zero engineering dependency) remains from that
>   original list of 4.
>
> §1/§19/§20 are left below **unmodified as the historical narrative of how this program reached
> its go-live decision** — they remain useful for understanding the reasoning trail. For **current
> status, always defer to §2 and this notice**, not to the prose in §1/§19/§20.
...
| P1-1 | Sécurité | `staff_profiles.salon_id` mass-assignment. **Deployed to production 2026-07-06** (Go-Live Phase 2): `20260704200000` applied — `pg_policies.with_check` re-read directly, confirms `salon_id` now pinned in `staff_own_profile_update` exactly as dr-scratch-tested. | P1 | None — fully closed | None | 15 min — **done** | `CP2_DEEP_SECURITY.md`, `docs/go-live/PHASE_2_MIGRATION_DEPLOYMENT_REPORT.md` | Fermé (preuve) |
...
| P1-3 | Infrastructure / DR | Recurring automated backup mechanism, **fully live in production 2026-07-06** (Go-Live Phase 3): `create-platform-backup` deployed and triggered for real — 79 tables, 258 rows, 94,879 bytes exported in one real run, artifact confirmed retrievable from `kynza-backups` storage. A real restore-verification was performed against `kynza-dr-scratch` (never production) using this exact production artifact — the 2-row `salons` table's real backed-up content was restored into a throwaway verification table and confirmed byte-for-byte identical, then cleaned up. RPO now bounded at ≤6h going forward. | P1 | None — fully closed | None | 2-4h — **done** | `docs/go-live/PHASE_3_PRODUCTION_OPERATIONS_REPORT.md` | Fermé (preuve) |
...
| P1-9 | Fiabilité / Concurrency | Two real concurrency bugs: `OfflineSyncCoordinator.flush()` double-applies every queued mutation on concurrent flush; `run-scheduled-actions` double-processes a pending automation action on concurrent cron invocation. **Server fix deployed to production 2026-07-06**: `20260705100000_cp0_concurrency_atomic_claims.sql` applied. **Re-proven under genuine concurrent load in production** (Go-Live Phase 3): 2 truly-parallel HTTP calls to `claim_pending_action_runs()` (fired via background shell jobs, both ~1.6s round-trip, overlapping in wall time) raced for the same test row — exactly one received it, the other got an empty result. Real race, not simulated; test data cleaned up immediately after. Client fix (`AtomicClaimService`) still ships with the next app release, unchanged. | P1 | High once real traffic exists, now mitigated server-side | Client fix: none (ships with app). Server: none — deployed | Client: ships automatically next release. Server: **done** | `CONCURRENCY_REPORT.md` (CP0), `docs/go-live/PHASE_2_MIGRATION_DEPLOYMENT_REPORT.md`, `docs/go-live/PHASE_3_PRODUCTION_OPERATIONS_REPORT.md` | Fermé (preuve) (server); Corrigé, awaiting next release (client) |
...
| P1-12 | Observabilité | Payment-failure dashboard + alerting mechanism, **fully live in production 2026-07-06** (Go-Live Phase 3): `check-system-alerts` deployed, `kynza-check-system-alerts` cron job registered (`*/5 * * * *`, migration `20260706150000`). **All 3 alert categories triggered for real** against production (controlled, clearly-marked test data, cleaned up immediately after): edge-function-error-rate (80% over threshold), sync-queue-depth (40 min stale), payment-failure-rate (80% over threshold) — all 3 correctly recorded in `system_alerts` and confirmed via direct read, then deleted along with their trigger data. WhatsApp dispatch itself remains best-effort/inert (`WHATSAPP_TOKEN` is an External Go-Live Dependency) — alert *recording* does not depend on it. | P0 | None — fully closed (dispatch channel is external, tracked separately) | None | 30 min — **done** | `docs/go-live/PHASE_3_PRODUCTION_OPERATIONS_REPORT.md` | Fermé (preuve) |
...
| P2-1 | Sécurité | `create_default_document_templates(p_salon_id)` — anon-callable, zero caller/role validation. **Deployed to production 2026-07-06** (Go-Live Phase 2): `20260704210000` applied — **live exploit re-attempted against production itself**: anon RPC call now returns `403 {"code":"P0001","message":"forbidden"}` (was previously unrestricted). | P2 | None — fully closed | None | 15 min — **done** | `CP2_DEEP_SECURITY.md`, `docs/go-live/PHASE_2_MIGRATION_DEPLOYMENT_REPORT.md` | Fermé (preuve) |
| P2-2 | Sécurité | `calculate-commission` — any authenticated user of any salon can read another salon's exact commission amount for a guessed `booking_id`; no salon-ownership check. **Closed with Production Evidence (Backend Production Closure CP1-CP4, 2026-07-06)**: redeployed (commit `2c13f47`, bundle hash `c48517d99...97271f` byte-identical to `kynza-dr-scratch`'s). Real before/after exploit reproduced live against production using a temporary test identity (created, exercised, fully deleted — zero residue): a cross-tenant caller now gets `403 {"error":"forbidden"}` against a real completed booking; a same-tenant caller still succeeds (`200`, zero side effects). Rollback: redeploy the pre-`2c13f47` version (`git show 2c13f47~1:...`) or `supabase functions delete calculate-commission`. | P2 | None — fully closed | None | Redeploy function — **done, production-evidenced** | `docs/backend-production-closure/CP1_PRE_DEPLOYMENT.md`, `CP2_SAFE_REDEPLOYMENT.md`, `CP3_LIVE_VALIDATION.md`, `CP4_PRODUCTION_PARITY.md` | Fermé (preuve) |
| P2-3 | Sécurité | `run-scheduled-actions`/`schedule-reminders` cron-secret gate, **fully enforced in production 2026-07-06** (Go-Live Phase 3): both functions redeployed with the header-check code. Re-verified live: an anon-only call (no `X-Cron-Secret`) to both functions now returns `403 {"error":"forbidden"}`; a call with the correct secret succeeds (`200`). | P2 | None — fully closed | None | 30 min — **done** | `docs/go-live/PHASE_3_PRODUCTION_OPERATIONS_REPORT.md` | Fermé (preuve) |
...
| P2-5 | Sécurité | Oversized payload (2MB JSON) hangs an Edge Function 45+ seconds; no function has a body-size limit. **Root-caused (P2-5 RCA, 2026-07-07)**: `Content-Length` unreliable across the Supabase gateway→Deno-isolate hop, causing the old header-only guard's own documented fallback to admit an unbounded body. **Fixed (P2-5 ECR, 2026-07-07)**: `readBodyGuarded()`, a shared streaming byte-count guard (`supabase/functions/_shared/cors.ts`) depending only on bytes actually read from the stream, never on the header — deployed to all 16 affected functions, live in production. **Live-validated**: at every payload size the platform reliably delivers (confirmed up to ~208KB, more than double `MAX_BODY_BYTES`), the fix is 100% deterministic across 15-30 consecutive real attempts per function, vs. 0-20% for the pre-fix code tested the same session (`docs/p2-5-ecr/CP3_TESTS.md` Section H, `docs/p2-5-ecr/CP5_VALIDATION.md`). **Closed with Engineering Evidence, scoped precisely**: the `Content-Length`-reliability mechanism is closed; a separate, newly-discovered platform ceiling for large request bodies (**P2-28**, below) is not part of this closure and was never in this ECR's scope. | P2 | Low (was Medium) — the diagnosed DoS mechanism is closed; residual risk is bounded by P2-28's separate ceiling, not this fix | None | Done — **root-caused, fixed, deployed, validated** | `docs/p2-5-rca/FINAL_RCA_REPORT.md`, `docs/p2-5-ecr/CP1_DESIGN_REVIEW.md` through `FINAL_CERTIFICATION.md` | **Fermé (preuve)** |
...
| P2-9 | Backend | Remote Config's 2 Edge Functions (`update-remote-config`, `rollback-remote-config`) gated on `role==='owner'` instead of `has_system_admin()`. **Closed with Production Evidence (Backend Production Closure CP1-CP4, 2026-07-06)**: both functions were absent from production entirely (404) before this closure — first-deployed (commit `d9c7613`). Real before/after reproduced live: the exact pre-fix bypass shape (`role='owner'`, `is_system_admin=false`) now gets `403`; a real system_admin succeeds end-to-end on both functions (update → version 2 → rollback → version 3), final config value re-read unchanged (`10`, zero net impact). Rollback: `supabase functions delete update-remote-config` / `... rollback-remote-config` (both didn't exist before this closure). | P2 | None — fully closed | None | Trivial code change — **done, production-evidenced (first deploy)** | `docs/backend-production-closure/CP1_PRE_DEPLOYMENT.md`, `CP2_SAFE_REDEPLOYMENT.md`, `CP3_LIVE_VALIDATION.md`, `CP4_PRODUCTION_PARITY.md` | Fermé (preuve) |
...
| P2-20 | Observabilité | No alerting/threshold code existed anywhere prior to P1-12; confirmed by repeated `grep` across 2 passes returning 0 matches before Resilience CP6 built one. **Closed with P1-12** (Go-Live Phase 3, 2026-07-06) — see that row. | P2 (superseded by P1-12's fix) | — | — | — | `MASTER_ISSUES_MATRIX.md` P2-20, `docs/go-live/PHASE_3_PRODUCTION_OPERATIONS_REPORT.md` | Fermé (preuve) (see P1-12) |
...
| P2-28 | Sécurité / Infrastructure | Request bodies roughly ≳210KB (a probabilistic band, not a clean threshold — evidence-backed as ~209,000-220,000 bytes mixed, consistently failing above ~220,000) have a substantial-to-near-total chance of never reaching the Edge Function isolate at all. **Discovered while validating P2-5's fix (P2-5 ECR, 2026-07-07)** — not part of that fix's scope. **Proven platform-level, not application code**: the *unmodified pre-ECR* code, given an honest, `curl`-computed, entirely accurate `Content-Length` header, hung 5/5 times at 210,000 bytes on production; the new streaming guard shows identical behavior at the same sizes. Old code and new code affected equally rules out either implementation as the cause. **Renumbered from `P2-22`** (Backend Governance Phase 1, 2026-07-07) — the originally-filed ID in `docs/remediation/MASTER_ISSUES_MATRIX.md` collided with this document's own pre-existing, unrelated, already-closed `P2-22` (row above, bulk-write trigger ceiling). All 16 affected Edge Functions confirmed safe against this ceiling with a wide margin: single shared `MAX_BODY_BYTES = 102,400` bytes constant (`supabase/functions/_shared/cors.ts`), no per-function override, live-verified at the exact byte boundary in production (`docs/final-doc-verification/BODY_LIMIT_AUDIT.md`) — over 106,000 bytes of margin below the first observed failure. | P2 | Medium — real, DoS-shaped, but no current application-level exposure given the wide safety margin above | Needs its own RCA with the same rigor as P2-5's; no tooling available to this program can see inside a hung invocation from outside it | Not started — Large/unknown until root-caused | `docs/p2-5-ecr/CP3_TESTS.md` Sections G-H, `docs/p2-5-ecr/CP5_VALIDATION.md`, `docs/remediation/MASTER_ISSUES_MATRIX.md` P2-28 (formerly P2-22), `docs/final-doc-verification/BODY_LIMIT_AUDIT.md` | Ouvert (newly discovered, not fixed, not fixable from application code alone based on evidence gathered so far) |
| P2-29 | Observabilité / Infrastructure | `auth.audit_log_entries` (GoTrue's native admin-action audit log) has **0 rows total in production**, not just for any one action — confirmed directly (`SELECT count(*) FROM auth.audit_log_entries` → `0`). **Discovered incidentally** while documenting a temporary test-identity's lifecycle during the Supabase Advisors review (Backend Governance / Advisors Review session, 2026-07-07, `docs/advisors-review/CP6_EXECUTION_LOG.md`) — pre-existing, present before this session, unrelated to any of the 6 Advisors alerts actually treated that session. **Not fixed, not investigated further** — this session's scope was the 6 classified Advisors alerts, not a new audit-log RCA. Two candidate explanations not yet distinguished: this GoTrue log mechanism may be disabled/unconfigured at the platform level for this project, or it may simply have never been exercised by any admin-console action (only Admin-API/SQL actions were performed, which this session's own evidence shows do not populate it either). Note: `system_admin_audit` (the *application-level* audit table for `grant_system_admin()`/`revoke_system_admin()`) is a separate, already-working mechanism (P2-8, Fermé) — this finding is specifically about GoTrue's own built-in log, not a regression of P2-8. | P2 | Medium — no live exploit, but a real forensic/incident-response gap: no native trail of admin-console actions (password resets, manual user edits, etc.) if any occurred outside the application's own logging paths | Needs its own investigation (platform config vs. never-exercised) before any fix is proposed | Not started | `docs/advisors-review/CP6_EXECUTION_LOG.md` (RC-5c temporary-identity section) | Ouvert (newly discovered, not investigated, not fixed — constat only, per Rule 2/3 of the Advisors review prompt: no correction without a dedicated investigation first) |
...
| P3-15 | Sécurité | `get_staff_week_rank` had a loose anon EXECUTE grant. **Deployed to production 2026-07-06** (bundled in `20260704210000`, Go-Live Phase 2): `has_function_privilege('anon','get_staff_week_rank(uuid)','execute')` re-checked live against production → `false`. | P3 | None — fully closed | None | Included in P2-1's migration — **done** | `PHASE_2_SECURITY_FIXES.md`, `docs/go-live/PHASE_2_MIGRATION_DEPLOYMENT_REPORT.md` | Fermé (preuve) |
...
**Total distinct items in this table: 69** (1 P0, 13 P1, 27 P2, 21 P3, 7 R — some rows carry two IDs where a later pass re-confirmed rather than duplicated a finding). **Updated 2026-07-07 (Backend
Governance Phase 1)**: added P2-28 (platform body-delivery ceiling, discovered validating P2-5's
fix, renumbered from a collision with this table's own pre-existing P2-22 — see the notice above
§2); P2-5's row updated to `Fermé (preuve)` with its actual root-cause and fix evidence.
