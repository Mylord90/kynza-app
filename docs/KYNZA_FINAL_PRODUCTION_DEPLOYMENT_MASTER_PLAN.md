...
| P2-5 | Sécurité | Oversized payload (2MB JSON) hangs an Edge Function 45+ seconds; no function has a body-size limit. **Redeployed to all 16 functions (Backend Production Closure CP2, 2026-07-06)** — code-level parity with git `HEAD` confirmed (all 16 carry `checkBodySize()`, fresh deploy timestamps). **NOT certified closed — a real, reproducible reliability gap was found while validating (CP3), not assumed fixed by the redeploy alone**: 27 real live attempts across production and `kynza-dr-scratch`, two independent HTTP clients, returned the correct `413` only 3 times — including one function (`accept-invitation`) that succeeded once then failed 3 more times on an identical request. Ruled out (with evidence): curl-specific artifact, cold-start-only effect, general outage, absent code. Root cause (candidates: multi-instance edge deployment with inconsistent `Content-Length` propagation, an upstream proxy/CDN not always forwarding the header, a runtime race) is undiagnosed — genuinely open engineering investigation, **reclassified from Category B (Ops) to Category A (Engineering)** as a direct result of this finding, since a redeploy alone cannot close it. | P2 | Medium (mild DoS) — intermittently still exploitable in production, confirmed by direct re-test | Dedicated investigation into why the guard doesn't trigger reliably | Redeploy done; **root-cause diagnosis + fix not done, Large/unknown until investigated** | `docs/backend-production-closure/CP3_LIVE_VALIDATION.md`, `CP4_PRODUCTION_PARITY.md` | Ouvert (re-scoped — redeployed but not reliably validated) |
