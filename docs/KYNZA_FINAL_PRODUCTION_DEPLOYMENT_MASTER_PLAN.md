...
| P2-2 | Sécurité | `calculate-commission` — any authenticated user of any salon can read another salon's exact commission amount for a guessed `booking_id`; no salon-ownership check. Fix drafted (code patch, no migration needed), live-tested before/after on dr-scratch. **Confirmed still NOT deployed to production** (Go-Live Phase 4 verification, 2026-07-06): `calculate-commission` was never among the functions Phases 1-3 redeployed (only `create-platform-backup`/`check-system-alerts`/`run-scheduled-actions`/`schedule-reminders` were) — directly evidenced by this same function still hanging on an oversized body in the P2-5 re-test, proving its deployed code predates *any* of this campaign's fixes, not just this one. | P2 | Medium, CVSS≈5.3 — confirmed still exploitable in production | None | Redeploy function — **not done** | `CP4_EDGE_FUNCTION_REVERIFY.md`, live re-test 2026-07-06 | Corrigé-non-déployé |
...
| P2-5 | Sécurité | Oversized payload (2MB JSON) hangs an Edge Function 45+ seconds; no function has a body-size limit. **Fixed (Enterprise Final 100 CP2)**: `checkBodySize()` added to `_shared/cors.ts`, wired into all 16 functions that parse a JSON body. Live-tested on dr-scratch: 200KB body → 413. **Re-confirmed still NOT live in production** (Go-Live Phase 4 verification, 2026-07-06): a real 300KB POST to `calculate-commission` timed out past 20s with no response, matching the exact pre-fix hang this row describes — Phases 1-3 never redeployed this function (they redeployed only `create-platform-backup`/`check-system-alerts`/`run-scheduled-actions`/`schedule-reminders`). Genuinely still open, not silently assumed closed. | P2 | Medium (mild DoS) — confirmed still exploitable in production | None | Small, per-function — **16 functions still need redeploy** | `PHASE_6_SECURITY_OFFENSIVE.md`, `CP2_SECURITY.md`, live re-test 2026-07-06 | Corrigé-non-déployé |
...
| P2-9 | Backend | Remote Config's 2 Edge Functions (`update-remote-config`, `rollback-remote-config`) gated on `role==='owner'` instead of `has_system_admin()`. Fixed and live-tested on dr-scratch (Master Plan Execution CP2). **`has_system_admin()` dependency is now live in production** (Go-Live Phase 2, migration `20260704120000`), but neither function was among those Phases 1-3 redeployed — confirmed by deployment history, not directly re-tested live this pass (would require a real owner JWT against production, not attempted). Genuinely still open. | P2 | Medium — DB dependency now met, function redeploy still not done | None remaining | Trivial code change — **not done** | `MASTER_ISSUES_MATRIX.md` P2-9, `docs/master-plan-execution/CP2_DEPLOYMENT_READY.md` | Corrigé-non-déployé |
