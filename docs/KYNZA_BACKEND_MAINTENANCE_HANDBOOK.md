...
**6. Supabase Advisor warning counts grew between passes on the same rule — same "growing count" pattern, not a discrepancy.**
`docs/certification/PHASE_2_DATABASE_OPTIMIZATION.md` (2026-07-04/05) reported 83 `auth_rls_initplan` and 205 `multiple_permissive_policies` warnings; `docs/advisors-review/CP1_COLLECTE_BRUTE.md` (2026-07-07) reports 108 and 227 respectively for the *same two rules*. Likewise `unused_index`: 50 (`PHASE_2_DATABASE_OPTIMIZATION.md`, cited via P3-19) vs 93 (Advisors CP1). The most consistent explanation, following the Master Plan's own precedent for exactly this pattern (§5, "14→16→18→20 undeployed migrations — a growing count, not a discrepancy"): the Group 2 feature-migration batch deployed 2026-07-06 added new tables/policies that the same Advisor rules now also scan, growing the base population these rules score against. **No session has explicitly re-run the Advisor and attributed the delta to specific new objects** — this attribution is this Handbook's own reasonable inference from the timeline, not a re-verified fact, and is marked as such rather than stated as settled. RC-4's 15 unindexed FKs (Advisors, on the new feature tables) are a **distinct, non-overlapping set** from P2-15's earlier 32 (already fixed, on the original tables) — confirmed by comparing table names in `CP5_PLAN_CORRECTION.md` Fiche 4 against `DATABASE_ARCHITECTURE.md` §4; not double-counted anywhere in this Handbook.
...
| Technology | Role |
|---|---|
| Flutter 3.22+ / Dart 3.4+ | Mobile client (Android live; iOS not started, Checkpoint 12) |
...
Notable RPCs beyond `has_role()`/`check_permission()`: `evaluate_feature_flag(key)`, `is_maintenance_active()`, `check_app_version(platform, version_code)`, `mark_invoice_paid()`, `add_loyalty_stamp()`/`redeem_loyalty_reward()`, `create_entity_version()`, `check_rate_limit()` (fails open — ADR-0001), `claim_pending_action_runs()` (ADR-0002 Shape A), `grant_system_admin()`/`revoke_system_admin()`, `get_supabase_dashboard()`/`get_bi_revenue()`/`get_audit_security_trail()` and the ~29 other `get_*`/`v_*` BI/audit/dashboard accessors gated by `has_system_admin()` post-RC-5c (Checkpoint 5.4). A handful of RPCs are called **directly from Flutter**, bypassing any Edge Function (`evaluate_feature_flag`, `mark_invoice_paid`) — a deliberate, documented pattern for read-mostly or single-statement-atomic operations that don't need service-role privilege.
...
For each module: role, key tables/RPCs/functions, key screens, dependencies, risks/known gaps.
...
| 4 | Supabase Advisor warning counts on unchanged rules | `docs/certification/PHASE_2_DATABASE_OPTIMIZATION.md` (2026-07-04/05): 83 `auth_rls_initplan`, 205 `multiple_permissive_policies`, 50 `unused_index` | `docs/advisors-review/CP1_COLLECTE_BRUTE.md` (2026-07-07): 108, 227, 93 respectively | No session has explicitly attributed this growth to specific new objects — the Handbook's Checkpoint 0.2 item 6 offers the most consistent explanation (new Group 2 feature tables/policies) as a reasoned inference, not a re-verified fact; a future Category B session re-running the Advisor with object-level diffing would settle this definitively |
