...
**`Firebase.initializeApp()` failure at boot — a blind spot Crashlytics structurally can't cover itself** (identified during the Firebase Android verification session, `docs/android/FIREBASE_ANDROID_CERTIFICATION.md`, Checkpoint 5): if Firebase itself fails to initialize in `main.dart`'s `_bootstrap()`, `FirebaseCrashlytics.instance` doesn't exist yet either, so the failure can't be reported through the normal path. Firebase is observability + push only here (nothing booking/payment-critical depends on it), so the chosen behavior is graceful degradation, not a blocking error screen: the app continues booting without Crashlytics/FCM/Performance for that session (`firebaseAvailable` flag in `main.dart`), the raw error goes to `debugPrint` immediately, and — since Crashlytics can't record its own outage — the error is written to a small unencrypted Hive box (`FirebaseInitFailureLog`, `lib/core/services/firebase_init_failure_log.dart`). On the **next** boot where Firebase does come up, `CrashReportingService.reportPendingInitFailure()` reports it to Crashlytics as a non-fatal (`reason: 'firebase_init_failure_catch_up'`) and clears the local entry — closing the gap almost for free rather than leaving a real Firebase outage silently invisible.

**Test coverage — precise, not overstated**: `test/unit/firebase_init_failure_log_test.dart` (+5 tests) proves two things directly — `FirebaseInitFailureLog.record()/peek()/clear()`'s Hive round-trip, and that `reportPendingInitFailure()`, given an entry already sitting in the box, calls its injected `recordError` seam with the right exception/`reason: 'firebase_init_failure_catch_up'`/`fatal: false` and then clears the entry. The injection point is `reportPendingInitFailure()`'s own `recordError` parameter — the test never calls `Firebase.initializeApp()`, real or mocked (no `setupFirebaseCoreMocks`-style Firebase platform mocking exists anywhere in this suite, per that test file's own note and `proxipay_repository_impl_test.dart`'s precedent for why). **Two things remain genuinely unverified, by either an automated test or a real device run**: (1) the `try`/`catch` in `main.dart`'s `_bootstrap()` around `Firebase.initializeApp()` — that a real init failure actually reaches `debugPrint` + `FirebaseInitFailureLog.record(error)` from that exact call site, never exercised because `_bootstrap()` itself isn't unit-testable (it drives `Hive.initFlutter()`/`Supabase.initialize()`/`runApp()`); (2) the production default of `reportPendingInitFailure()` — the un-injected lambda that calls the real `FirebaseCrashlytics.instance.recordError(...)` — only the test's substitute callback has ever actually run. This is the critical path of the whole mechanism and it is currently **not covered at all** — not by the unit test, not by a device run (attempted three times this session; the dev machine's build environment could not sustain a reliable Android build or even a plain `flutter analyze` at some points — see the certification doc's environment notes). Tracked as an open follow-up: a real device pass (deliberately breaking `google-services.json`, confirming the local log write, then restoring it and confirming the Crashlytics catch-up) once the dev machine's memory pressure is resolved — not attempted again prematurely, since a retry on an already-unstable machine would likely just reproduce the same crashes without proving anything.

**Why the device pass couldn't complete this session — corrected diagnosis**: `android/gradle.properties`' `org.gradle.jvmargs` was originally `-Xmx8G` on a dev machine with only ~7G physical RAM — that specific mismatch was real and is fixed (now `-Xmx4G`, a legitimate correction on its own terms, verified once at `-Xmx3G` during the certification session). But that fix is **not** what was blocking this pass, and should not be read as having resolved the instability observed here: builds continued failing with native OOM at `-Xmx4G` and even `-Xmx2G`, and separately a plain `flutter analyze` (which takes no heap argument from `gradle.properties` at all) also crashed the Dart VM with an out-of-memory error. The actual cause was **general system-wide memory over-commitment from concurrently-running processes** (Android Studio, multiple redundant VSCode Java-language-server processes measured at ~1.5G combined, antivirus, etc.) exhausting the Windows commit limit (RAM + pagefile) — not any single Gradle setting. Killing the redundant language-server processes freed ~1G but a build still failed afterward. A machine reboot got `flutter analyze`/`flutter test` to pass once each, but with free RAM still under 1G at idle immediately after — one clean run, not a demonstrated sustained fix. **Status: open**, to be re-assessed once Mylord confirms the system-wide memory pressure is durably resolved, not assumed fixed by the reboot or the `gradle.properties` correction alone.
